...
check "the running orchestrator has OpenCode on its PATH (rebuilt after M82)" bash -c 'docker exec jarvis-orchestrator sh -c "HOME=/tmp/home opencode --version" 2>&1 | grep -E "^[0-9]+\.[0-9]+"'
...
check_sh "on the house: a coding job runs end to end through the orchestrator" \
    'LIVE_ONLY=coding-fix-failing-tests timeout 1800 bash scripts/verify/live_interaction.sh --full 2>&1 | grep -v onnxruntime | tail -4'
verify_end
