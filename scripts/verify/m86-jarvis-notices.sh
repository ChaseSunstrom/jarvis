...
print("offer parsed")
'
check_pytest "the narrate suite: rules, ceilings, quiet hours, the offer asked and done only on a yes" 'cd jarvis-core && python3 -m pytest tests/test_sensors.py -q --timeout=120 --timeout-method=signal -k "narrat or offer or notice or held"'
check_pytest "what the narrator said or asked is on the notifications record, once when asked and once when it acted" 'cd jarvis-core && python3 -m pytest tests/test_notifications.py -q --timeout=120 --timeout-method=signal -k "narrator"'
