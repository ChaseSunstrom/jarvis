...
/**
 * Home Assistant Assist pipeline client over the WebSocket API — a faithful
 * Kotlin port of jarvis-web/src/lib/pipeline.ts, so the phone speaks the exact
 * same protocol as the browser HUD:
 *
 *  1. auth handshake (auth_required -> auth -> auth_ok)
 *  2. assist_pipeline/pipeline/list -> resolve the "Jarvis" pipeline id
 ...
 *     run-end, error.
 *
 * All callbacks are delivered on the main thread. Uses OkHttp (already on the
 * HA app classpath) for the socket; org.json for messages (no extra deps).
 */
class AssistPipelineClient(
    ...
    private fun absolute(pathOrUrl: String): String =
        if (pathOrUrl.startsWith("http")) pathOrUrl else haUrl.trimEnd('/') + pathOrUrl
