...
| `jarvis/n8n/check` | `{ok, detail, capabilities, summary}` — three lines, not one: the public API, the optional `/rest` login, and whether this instance's licence includes n8n's AI builder. Each carries a `reason` from a closed set and a sentence naming the thing to change, because "n8n: no" sends people to the wrong half |
...
| `jarvis/n8n/build` | `instruction`; hands a request to n8n's OWN AI builder and returns `{task}` at once. It runs in the background because the builder can stop to ask questions, and a question cannot be answered inside the turn that raised it — the questions arrive as ordinary approval cards. Refused with the reason when the instance's licence does not include the builder |
| `jarvis/n8n/transcript` | `task_id`; what the builder and the household said to each other. Console-only on purpose: it is prose composed by a different AI, and the model gets one sentence instead |
| `jarvis/n8n/health` | `workflow_id`; whether a workflow is actually working — credentials attached, switched on, and running. Reads run status and timing only, never `includeData`, which would be the contents of every email and invoice that went through it |
