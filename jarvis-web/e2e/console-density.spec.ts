...
test('an entity and its MANAGE button share a line on a desktop viewport', async ({ page }) => {
	...
	expect(button, 'the MANAGE button has no box').not.toBeNull();
	expect(controls, 'the entity row has no box').not.toBeNull();

	// Centres within half a row of each other. A tolerance rather than an
	// equality: the button is shorter than the row it sits beside, and flex
	// centring is not pixel-identical across platforms.
	const buttonMid = button!.y + button!.height / 2;
	const rowMid = controls!.y + controls!.height / 2;
	expect(
		Math.abs(buttonMid - rowMid),
		'the MANAGE button is not on the same line as the entity it manages'
	).toBeLessThan(controls!.height / 2 + 4);

	// ...and it is to the RIGHT of the controls, not stacked under them.
	expect(button!.x, 'the MANAGE button is not at the end of the row').toBeGreaterThan(
