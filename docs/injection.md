...
| reads that reach **outside** (`web_search`, `web_fetch`, `web_browse`, `web_crawl`, `read_page`, `feed_latest`; `OUTBOUND_READERS`) | runs | **held** — the URL is the way out for what a hostile page wants carried, and a person sees it first |
