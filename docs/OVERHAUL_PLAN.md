...
| M63 | **The dashboard shows the house** — the operator's "full functionality": widget kinds beyond graphs. One entity's state with its switch (the Devices row's `call_service`, live over `state_changed`); the newest readings by room (M57); a still from a camera through the vision integration's consent and audit (M56 — a `never` camera shows its refusal, never a frame); the next ISS pass and the moon (M58, "not fetched yet" when they are not); the newest moments. Three read commands (`jarvis/sensors/readings`, `jarvis/sky/summary`, `jarvis/vision/still`) in the contract, served by the mock; the kind first in the widget editor; a shipped House the console opens on; every saved layout still loads. | `m63-dashboard-widgets.sh`: contract ↔ server ↔ console kinds, the still's path, the shipped House, core tests for the commands, unit + render tests per kind, e2e (tile round trip, refused camera, live moment, kind picker), the inventory, pictures | M61 |
| M66 | **Ask and answer** — the operator's reports: the voice speaks the reply and the question; a question expires in five minutes and the answer then fails; an answer or a confirmation should be sayable. Done 26 Aug 22:28. | `m66-ask-and-answer.sh` | M43, M60 |
| M67 | **Settings under approval** — `list_settings` (Tier 1) and `change_setting` (Tier 3) over the console's settings registry. Done 26 Aug 22:05. | `m67-settings-tool.sh` | M54 |
...
| M69 | **The house is editable by voice** — `remove_entities` / `remove_device` under approval, sayable. Done 26 Aug 22:28. | `m69-editable-house.sh` | M66 |
...
| M71 | **Enrolment, complete** — the audit of voice enrolment end to end, every gap closed. Done 26 Aug 22:14. | `m71-enrolment.sh` | M35 |
...
| M73 | **Said, not shown** — markdown and symbols become words at the synthesiser's door. | `m73-spoken-form.sh` | M35 |
| M74 | **Speak after tools, sentence by sentence** — early speech resumes per segment; the tail as the last chunk. | `m74-speak-after-tools.sh` | M60 |
| M75 | **Research that reads in time** — fallback first after a failure, text-first fetch, bounded parallel reads. | `m75-research-in-time.sh` | M68 |
| M76 | **Jarvis in the middle, the tasks below** — the reactor centred; the task dock under it on the voice page. | `m76-voice-layout.sh` | M49 |
| M77 | **n8n: the house's workflows** — list/run/create/manage over n8n's API under the tier rules; the assistant endpoint as untrusted advice. | `m77-n8n.sh` | M47 |
| M78 | **One utterance, one turn** — a second listener's copy of the same words yields; `schedule` refuses a duplicate. | `m78-one-utterance.sh` | M43, M52 |
| M79 | **Not listening while you enrol** — an enrolment in progress makes every pipeline turn yield for the window. | `m79-enrolling-not-listening.sh` | M71, M78 |
| M80 | **Demo mode is a setting** — `demo.enabled`, applied live through the one delete path. | `m80-demo-mode.sh` | M67, M69 |
| M81 | **It knows what it can do** — a denied capability a tool provides is sent back for the call; the address pinned. | `m81-knows-what-it-can-do.sh` | M19, M43 |
| M82 | **A coding job says when nobody can run it** — a queued job with no worker names the command; the worker's state on the Code screen. | `m82-coding-worker.sh` | M19 |
| M83 | **Pull things up** — a `show` tool and a surface of movable panels around the instrument. | `m83-pull-things-up.sh` | M63, M76 |
...
| M65 | **Something to browse** — the operator's "no way to browse" the tools and MCP servers from Settings. The catalogue had no source by default (M47) and its button was inside a fold. One source ships by code: `bundled`, the package's own four skills as `file://` from wherever the package is — not a URL, so M47's rule holds as written (`DEVIATIONS.md` §21); `installed` on every browse entry; the source's error reported. The Catalogue section is first on the tools page, above the folds, filtered by the one search, one control per row through the existing two-step install, and one MCP line whose control opens the add-by-URL form; stdio stays in `configuration.yaml`. | `m65-catalogue.sh`: the index against each SKILL.md, the source wiring, browse on a fresh Jarvis, the placement read off the Svelte, the mock's `installed`, the inventory, e2e, pictures | M47, M55 |
| M64 | **The phone looks like the console** — the fourteen mismatches an audit found between the native screens and `jarvis-web/src`, each closed by copying the console: one sliding tab underline under the brand and the readout, hairline rows in one panel, APPROVE as the accent primary, the activity row's pulsing dot, one label recipe and a lint that sees every dp and tracking literal, danger-text for failures, the reactor resting in the accent, `ScreenTitle`, the four screen states (`ScreenStates.kt`), the settings section strip, the console's control geometry, flat bars, the graph's labels and memories, the stagger and reduced motion. Build, unit, lint, goldens — no device. | `m64-phone-look.sh`: a check per mismatch reading the Kotlin, the extended token lint's self-test, the mirrors, the gradle four, the goldens | M56, M63 |
