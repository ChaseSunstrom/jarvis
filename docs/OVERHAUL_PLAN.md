...
| M68 | **Search that works** — the configured SearXNG times out on every engine; try it, then the local default; say which engines answered. | `m68-search.sh` | M31 |
...
| M72 | **A coding job can create a repository** — the `/workspace` crossover mounted, writable, named. | `m72-workspace.sh` | M19 |
