...
| The orchestrator against a **real** running service | Automated + Containerised (M82's gate 6/6 on 27 Aug 2026, the 11:45 house: a coding job end to end through OpenCode 1.18.23, the Code screen naming the worker, the result naming the failed check) | M82's gate (`scripts/verify/m82-coding-worker.sh`) probes `POST /code_task` on the running container and reads its answer; the container is up and healthy on the house. The coding path itself waits on OpenCode reaching the image (BLOCKERS) — that half is Unproven. |
...
| A 2-second median round trip | **Missed, and reported** | Round trip (typed) — 3.84 s median, 21.52 s p95 over 156 turns on the 16:14 report run of 99 scenarios, 87 passed, intent 92.7 %, routing 96.2 %, containers clean — unit suites and svelte-checks ran beside it (lighter than the nineteenth's load; still the box's number); Round trip (typed) — 4.31 s median, 19.74 s p95 over 158 turns on the 15:04 report run of 99 scenarios, 85 passed, intent 91.6 %, routing 90.6 % — vitest, Playwright and Electron runs shared the box the whole time (the numbers are the box's); 3.98 s median, 14.15 s p95 on 27 Aug 2026 (the 13:48 report run of 99 scenarios, 88 passed, intent 93.5 %, routing 98.1 % — console builds and a Playwright run shared the box during it); 4.50 s and 22.79 s in the 12:25 run of 99, 80 passed, routing 92.5 % (a Python 3.12 suite, a gradle build and a Playwright run shared the box with it, so the times are the box's, not the house's); 2.95 s and 13.41 s in the 11:04 run of 99, 82 passed, routing 94.2 %; 2.51 s and 14.55 s in the 09:47 run of 98, 4.17 s and 21.4 s in that morning's run of 63; 15–20 s when first measured on 24 Aug, before M60/M70/M74) on this host (27 B model, no GPU, four shared vCPUs). `ISSUES.md` and `BLOCKERS.md` say what it would take |
...
| Every value cell on every Settings tab starts at one left edge, and the controls in a panel share one width (M107) | Automated | `jarvis-web/e2e/settings-layout.spec.ts` (2 tests at 1440×900, tolerance 2 px / 48 px) via `bash scripts/verify/m107-settings-line-up.sh` |
| The desktop app from a downloaded folder: the setuid helper is checked before `ready` and the process-sandbox switch set when it cannot be used; a closed console port draws a page naming the URL (M110) | Automated | `jarvis-desktop-app/tests/config.test.ts` (`sandboxHelperUsable`, 3 cases) + `e2e/unreachable.spec.ts` under xvfb, via `bash scripts/verify/m110-desktop-starts-from-a-folder.sh` |
| The voice tab's tasks are one-line briefs that expand on a click; the list scrolls inside itself and the page does not (M111) | Automated | `jarvis-web/e2e/tasks-brief.spec.ts` via `bash scripts/verify/m111-tasks-are-a-brief.sh` |
| A note or a page the house puts on the voice screen is one row at a side — title and first line — opened to the whole note by ⤢ and folded by ⤡ (M112) | Automated | `jarvis-core/tests/test_surface.py` (placement) + `jarvis-web/e2e/notes-brief.spec.ts` via `bash scripts/verify/m112-notes-are-a-brief.sh` |
| Markdown renders completely — tables with alignment, nested and task lists, fenced/indented code, nested quotes, setext headings, hard breaks, strikethrough, autolinks — with the source escaped first and links http(s) only (M113) | Automated | `jarvis-web/src/lib/markdown.test.ts` (10 cases) + `e2e/markdown.spec.ts` via `bash scripts/verify/m113-markdown-renders-completely.sh` |
| Every variable `.env.example` names is listed in SETTINGS › SYSTEM with its why; SET keeps a value in the house's store and applies it over the container's environment at the next boot, before configuration is read; secrets are masked until REVEAL; CLEAR forgets; RESTART JARVIS applies (M114) | Automated | `jarvis-core/tests/test_environment.py` (4) + `test_packaging` (the image carries the catalogue) + `jarvis-web/e2e/environment.spec.ts` (4) + the live probe `scripts/verify/m114_env_probe.py`, via `bash scripts/verify/m114-env-from-the-console.sh` |
| The two registries are read through an allowlist of three hosts; skills pinned to a commit, MCP servers one row per name with an https remote; hostile text quarantined; a stdio-only server skipped and counted (M108) | Automated | `jarvis-core/tests/test_registries.py` (19 tests on recorded slices of both registries) |
...
`activity.test.ts` and `enrolment.test.ts`; phone: `enrolment_flow_test.py` (18), `activity_mirror_test.py` (7),
...
| The phone screen: "Who is this" above the phrases, the household listed with FORGET each, FORGET EVERYONE, TEST MY VOICE naming who, a loading line; the strip's speaker row | Automated as a Python mirror and on the JVM / Unproven on a device | `enrolment_flow_test.py` (18), `activity_mirror_test.py` (7), `ActivityRowsTest`, the `voice-activity` golden (recorded and verified), `assembleDebug`, `lintDebug`; a device is ADT-021, ADT-052, ADT-053, ADT-054 |
...
| **A prompt reaches the person it was raised for, and the orb stays with it** | Automated *as a Python mirror* | `prompt_reaches_the_user_test.py` (20 checks). The twentieth is M98's: a Tier-3 action jarvis-core holds is raised on the phone's consent screen and answered with `jarvis/approve`. The nineteenth is M66's: a question the reply already says (`spoken`) is shown on the phone and not read out again — the operator heard every question twice. The eighteenth is the one that came back: clearing the prompt's buttons was first done with `View.GONE`, which fixed the touches by making the two surfaces mutually exclusive — any prompt going up took the orb off the screen, so Jarvis asking you something meant Jarvis disappearing while it asked, with a live conversation underneath and no surface showing it. The card now collapses to a badge at the top of the screen, opposite where a consent screen puts DENY/APPROVE, and stays visible; `FLAG_NOT_TOUCH_MODAL` passes the prompt's touches through on its own. Four defects behind one report. The Hey Jarvis surface was `android:noHistory`, so the consent prompt, a question and the permission trampoline each destroyed the conversation underneath on the way up and answering returned to nothing. `startActivity` returning proves nothing — a refused background start does not throw — and a full-screen intent degrades to a heads-up whenever the screen is on and unlocked, so on a phone in use nothing raised the prompt at all. `PolicyStore.setPolicy` had no caller outside the tests, so "may Jarvis do this without asking" was unanswerable. And an `ask` message was never spoken. Whether the prompt appears over a third-party app on a real phone is still **Unproven**. |
| **The phone shows what the house puts up** | Automated *as a Python mirror* | `surface_on_the_phone_test.py` (12 checks). The console drew the surface's panels and the task dock; the phone consumed neither (`jarvis_surface_changed` reached no Kotlin). The device channel now watches the surface the way it watches tasks and moments — subscribe first, list second — and the voice screen says each panel in a line and draws the dock's bars; the mirror pins the kinds to the server's KINDS and the frames to the server's names. Goldens `voice-surface`, `voice-task-dock`. |
| **The house can put a task on the phone** | Automated *as a Python mirror* | `phone_tasks_test.py` (16 checks). `TaskStore.import` existed and nothing called it — PHONE TASKS said tasks "arrive from jarvis-core" and none could. The way in is `import_tasks`, a tier-3 builtin phone action (the consent screen once; the store still screens each task and one with a confirm-tier step arrives switched off), shipped through `control_device` like a torch; the house's half is the `phone-tasks` skill. The mirror pins the three ends to each other: the action's tier and registration, the skill's trigger ids to `TriggerIds` and its step types to `StepType`, its example's action ids to the builtin table. `TaskActionsTest` (JVM, 4) parses the skill's own example into a task; the M98 gate's fake phone on the house receives the bundle the model wrote. |
...
|---|---|---|
| M84 — Jarvis volunteers a briefing | Automated + Containerised (8/8 on 27 Aug 2026, the 10:21 house) | `scripts/verify/m84-briefing.sh`; `test_features.py -k briefing` (20); scenario `briefing-on-demand` |
...
| M92 — The house by voice | **Built** — 18/20 on the seventeenth's house (27 Aug 2026; 13/20 on the sixteenth): ask-which-light, house-cover, the fan and switch pass; the two left are "clear the screen" claimed with no call, a guard fix on the tree | `m92-the-house-by-voice.sh`; ten scenarios; rig tests 49 |
...
| M94 — In here | **Ticked** (7/7 on the seventeenth's house, 27 Aug 2026: a registered device is told its own name, and "turn off the lights in here" typed from a tablet filed in the kitchen turns off the kitchen — typed turns run on the device's socket since aa8992d) | `m94-in-here.sh`; `test_llm.py`, `test_voice.py`; scenario `in-here-by-voice` (the sixteenth) |
| M95 — Jarvis reads its own record | Automated + Containerised (5/5 on 27 Aug 2026, the 11:45 house) | `m95-reads-its-own-record.sh`; `test_notifications.py` (+3), `test_llm.py` (+2); scenarios `explain-yourself`, `what-did-you-tell-me` |
| M96 — Stop means stop | **Ticked** — 5/5 on the eighteenth's house (27 Aug 2026): a stop a second and a half into a typed turn over the socket ends the run `interrupted`, the probe stops on the first streamed token | `m96-stop-means-stop.sh`; `test_api.py` (+1); scenario `stop-means-stop` (the sixteenth) |
...
| M102 — Jarvis learns from its own mistakes | Automated + Containerised (6/6 on 27 Aug 2026, the 13:17 house) | `m102-learns-from-mistakes.sh` |
...
| M105 — The gate says no to a voice that is not yours | **Built** (68 speaker tests; the console's TEST shows the blocks and names the refusing one — `enrol.spec.ts` 8/8; the live half waits for a house built from this tree and a quiet rig) | `m105-not-your-voice.sh`; `test_speaker_gate.py -k "wrong_pitch or cold_morning or veto"`; `enrolment.test.ts` |
