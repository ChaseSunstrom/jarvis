...
| **On-device transcription and the speaker gate together** | **Not implemented, and said so on the screen** | They do not compose. The check runs on the server, on the audio; a turn the phone transcribes locally sends words rather than sound, so it is not verified. The settings screen says to leave on-device transcription off while the gate enforces. Closing it means porting `jarvis/voice/dsp.py` and the embedding to Kotlin so the phone can verify locally, with a shared fixture pinning numeric parity. That work has not been done. |
