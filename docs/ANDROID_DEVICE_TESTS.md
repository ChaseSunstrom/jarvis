...
| ADT-039 | Goldens | Re-record the Robolectric goldens (`./gradlew recordRoborazziDebug`) after looking at the voice screen with its strip and graph, and commit them | The build, lint and JVM tests run here (M08's toolchain); whether the new screen's golden is the right picture is an eye's call | M61 |
