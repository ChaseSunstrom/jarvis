...
| ADT-047 | Chrome | On the console screen, switch HOUSE → SETTINGS → HOUSE: the one underline must slide between the tabs over about a quarter of a second, not vanish and reappear, and the fifth tab must scroll into view with the edge fading to the ground rather than clipping | The slide is a `ValueAnimator` against a real vsync and the fade is the platform's edge compositing; Robolectric records one still frame of both | M64 |
| ADT-048 | Reactor | Open the app and leave it: the resting instrument must be one cyan — the level arc, rim and iris in accent-deep, the dot in the accent — with no indigo or purple anywhere in the lens, on an AMOLED panel at real brightness | The idle blend crosses `COLOR_BLEND_MS` on a real clock and the lens's radial stops are gamma-sensitive; the JVM golden proves the tokens, not the panel | M64 |
| ADT-049 | Motion | Set the animator duration scale to 0 in developer options (and separately the transition scale), reopen the app: the reactor must hold still while still responding to a voice, a running tool's dot must not pulse, an indeterminate task bar must show its segment parked in the middle rather than sweeping, and rows must appear without entering | `Settings.Global` is read on attach from a real content provider; the JVM cannot flip it under a running view | M64 |
| ADT-050 | Settings | On the phone's settings screen, tap each segment of the section strip and confirm the column scrolls to that heading, then scroll by hand and confirm the current segment follows the heading under the strip | The scroll listener and `smoothScrollTo` run against the real column's laid-out tops; the golden shows the strip alone | M64 |
| ADT-052 | Voice identity | On the phone, type a second name in "Who is this", read three phrases, and confirm the Enrolled list shows both people with their own counts, that the phrase list restarted at the top for the new name while the first person's did not, and that FORGET on one row removes only that person | Real speech through a real microphone; the JVM has no `AudioRecord` and the mirrors read the Kotlin as text | M71 |
| ADT-053 | Voice identity | With two people enrolled and `mode: enforce`, have each say something and then a third person: TEST MY VOICE must name each enrolled speaker, and the stranger's turn must be refused with a `speaker` row on the phone's strip reading "not recognised · refused · nearest …" | Whether two real voices separate is the verifier's accuracy on human speech, which no synthetic test settles (`verification.md`, "REAL human speech"); the strip's row is driven by the real bus | M71 |
| ADT-054 | Voice identity | Turn off the phone's network mid-screen and reopen TEACH JARVIS MY VOICE: the status line must read "Asking Jarvis who is enrolled…" and then "Could not reach Jarvis", never a blank screen with grey buttons | The loading and offline moments are a real socket's timing; Robolectric answers instantly | M71 |
| ADT-051 | Screens | Compare `screen-states`, `section-strip`, `console-frame`, `voice-activity` and `settings-fields` with the same moments on a real device beside the console's `docs/ui-review/*/mobile.png` at arm's length; note any place the phone still reads as a looser copy | Whether two surfaces read as one product is a person's call, not a pixel comparison | M64 |
