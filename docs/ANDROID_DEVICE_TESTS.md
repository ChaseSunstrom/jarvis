...
| ADT-039 | Build | `./gradlew assembleDebug testDebugUnitTest lintDebug verifyRoborazziDebug` on a machine with the SDK | This host has no Android SDK (CLAUDE.md); the Kotlin M61 added is mirrored in Python but has not been compiled here | M61 |
