...
- **M102 — Jarvis learns from its own mistakes.** The guard's refusals and a stopped run are on the trace; once a night a review reads the day's failures, keeps a lesson as a note and a card, and "what did you get wrong today?" is answered from the record rather than reconstructed.
- **M104 — Jarvis proposes a routine.** A miner over the recorder's history finds an entity put in the same state in the same quarter-hour on enough distinct days — not by an automation, never an unlock — and proposes it once a morning as a card and a question; a spoken yes makes the automation through the same door the console uses, a no is remembered for thirty days. `routines.propose/accept/decline`, the `proposed_routines` tool.
- **M101 — one shape for every payload.** A script connects to the house and to the console's mock, sends the twenty-two list commands both answer, and reports every key the server sends that the mock's rows lack — eleven the day it was written, none now, so the console's tests can no longer pass on a payload the real console never sees. The Code screen names its worker (the orchestrator's backend and OpenCode version, or that the orchestrator is off); Readings says when no sensor has a room; the error page's way home is the devices page.
...
- **M86 — Jarvis notices.** A door left unlocked after the configured hour, a sensor stuck for longer than its kind allows, a device gone unavailable: one nudge per thing per night, with the action offered as a held request a spoken yes resolves, and `recent_events` for "what happened?". The default rules ship in configuration.yaml, switchable per kind. Since 27 Aug an offer is a held question — on the console's bar, on a paired phone, on the notifications record, answered by a spoken yes to any surface — rather than a device-only ask that nobody saw when no phone was connected.
- **M97 — timers, routines read back, what's new.** "Set a fifteen-second timer for the pasta" makes `timer.pasta`: it counts on the house's clock, "how long is left?" is read from it, it chimes on the device that asked and leaves a card, and "cancel the tea timer" is a service call by name (a timer running across a restart comes back finished, saying it was interrupted). A routine authored by voice is read back and listed; a tool Jarvis authors, a skill or an MCP server it gains is said once and "what's new?" answers from that record.
- **M99 — the console keeps up.** A task that failed or was cancelled has RETRY on its card and its page; Areas, the scheduled jobs and every Settings row follow the house without a reload (a room made by voice, a firing, a `change_setting` from the phone); a phone gets a room on the Devices page, through the device-registry entry jarvis-core now files it under, and `device_of` reports it; token created/last-used, an automation's reach, a code repo's worker and permissions and a memory's expiry are shown. The mock carries every key it lacked.
...
- **M60 — intelligence and speed.** The wait on a voice turn was never the model (≈75 tok/s); it
  ...
  enough wants) and the MODELS panel says so instead of calling it idle. `voice: think: false`
  skips the reasoning block on spoken turns (`think=False` reaches llama.cpp and vLLM as the chat
  template's `enable_thinking` switch) — measured at 3.1 s median against 5.9 s and 87 % intent
  against 93 %, so it ships on and the switch is there for a house that wants the speed. A turn that makes the same calls three rounds running is
