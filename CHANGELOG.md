...
- **M86 — Jarvis notices.** A door left unlocked after the configured hour, a sensor stuck for longer than its kind allows, a device gone unavailable: one nudge per thing per night, with the action offered as a held request a spoken yes resolves, and `recent_events` for "what happened?". The default rules ship in configuration.yaml, switchable per kind.
